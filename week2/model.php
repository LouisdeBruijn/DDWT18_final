...
function get_serie_table($pdo, $series){
    ...
    foreach( $series as $key => $value){
        ...
        <tr>
            ...
            <th scope="row">'.get_username($pdo, $value['user']).'</th>
 ...
 */
function add_serie($pdo, $serie_info, $user_id){
 ...
 * @return array
 */
function update_serie($pdo, $serie_info, $user_id){
    /* Check if all fields are set */
    if (
        empty($serie_info['Name']) or
        empty($serie_info['Creator']) or
 ...
 * @return array
 */
function remove_serie($pdo, $serie_id, $user_id){
    /* Get series info */
...
/**
 * Count the number of users that are active on Series Overview
 * @param object $pdo database object
 * @return boolean
 */
